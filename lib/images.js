...
const getImgSrcset = (section, imgName, imgSizes, imageType) => {
	const reducerFn = (accumulator, currentValue, currentIndex, array) => {
		const stringon = `/img/posts/${section}/${imgName}_${
			currentValue.width
		}${`.${imageType || 'jpg'}`} ${currentValue.width}w`;
